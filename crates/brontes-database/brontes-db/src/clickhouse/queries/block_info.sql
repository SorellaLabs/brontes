WITH
    ...
    relay_bids AS (
        ...
        WHERE block_number = get_block_number AND value != 0 AND relays.proposer_fee_recipient != ''
    ...
    relay_payloads AS (
        ...
        WHERE block_number = get_block_number AND value != 0 AND relay_payloads.proposer_fee_recipient != ''
        GROUP BY block_number, block_hash
    ),
    raw_blocks AS (
        SELECT
        ...
        WHERE block_number = get_block_number AND valid = 1
        GROUP BY block_number, block_hash
    ),
