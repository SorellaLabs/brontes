...
mod tests {
    ...
    //     dotenv::dotenv().ok();
    //     let brontes_db_path = env::var("BRONTES_DB_PATH").expect("No
    // BRONTES_DB_PATH in .env");     Libmdbx::init_db(brontes_db_path,
    // None) }
    //
    ...
    //     #[cfg(feature = "local")]
    //     let tracer = {
    //         let db_endpoint = env::var("RETH_ENDPOINT").expect("No db
    // Endpoint in .env");         let db_port =
    // env::var("RETH_PORT").expect("No DB port.env");         let url =
    ...
    //         let client = TracingClient::new(Path::new(&db_path), max_tasks as
    // u64, executor.executor());         handle.spawn(executor);
    //         Box::new(client) as Box<dyn TracingProvider>
    //     };
    //
    //     let call = Box::new(|_: &_, _: &_| true);
    //
    //     TraceParser::new(libmdbx, call, Arc::new(tracer),
    // Arc::new(metrics_tx)) }
    //
    // async fn initialize_tables(tables: &[Tables]) ->
    // eyre::Result<Arc<Libmdbx>> {
    //
    //     let db = init_db()?;
    //     let rw = LibmdbxReadWriter(db);
    //     let clickhouse = Clickhouse::default();
    //     let (a, b) = unbounded_channel();
    //     let trace_parser =
    // init_trace_parser(tokio::runtime::Handle::current(), a, &rw, 10);
    //
    //     let db_initializer = LibmdbxInitializer::new(db.clone(),
    // Arc::new(clickhouse), trace_parser.into());     db_initializer.
    ...
    //    }
    // */
    // async fn test_tokens_decimals_table(db: &Libmdbx, print: bool) ->
    // eyre::Result<()> {     let tx = db.ro_tx()?;
    ...
    // }
    //
    // async fn test_address_to_tokens_table(db: &Libmdbx, print: bool) ->
    // eyre::Result<()> {     let tx = db.ro_tx()?;
    ...
    // }
    //
    // async fn test_address_to_protocols_table(db: &Libmdbx, print: bool) ->
    // eyre::Result<()> {     let tx = db.ro_tx()?;
    ...
    // }
    //
    // async fn test_cex_mapping_table(db: &Libmdbx, print: bool) ->
    // eyre::Result<()> {     let tx = db.ro_tx()?;
    ...
    // }
    //
    // async fn test_metadata_table(db: &Libmdbx, print: bool) ->
    // eyre::Result<()> {     let tx = db.ro_tx()?;
    ...
    //     }
    // */
    // async fn test_pool_creation_blocks_table(db: &Libmdbx, print: bool) ->
    // eyre::Result<()> {     let tx = db.ro_tx()?;
