...
(
    ...
    WHERE 
        (
            (bt.timestamp <= 1691758835875) AND (bt.timestamp > 1691758835875 - 10000000000)
        )
    ...
    WHERE 
        (
            (bt.timestamp <= 1691758837457) AND (bt.timestamp > 1691758837457 - 10000000000)
        )
        AND substring(bt.symbol, -4) = 'USDT'
    GROUP BY 
        address
...
WHERE (relays.block_number = 17891804) AND blocks.valid = 1