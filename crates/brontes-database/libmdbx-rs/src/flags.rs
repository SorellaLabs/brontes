...
pub enum SyncMode {
    ...
    /// the last committed transaction. I.e. it preserves the ACPI
    ...
    ///   read transaction since prevents reuse of pages freed by newer write
    ///   transactions, thus the any data changes will be placed in newly
    ///   allocated pages.
    /// - To avoid rapid database growth, the system will sync data and issue a
    ///   steady commit-point to resume reuse pages, each time there is
...
pub struct EnvironmentFlags {
    pub no_sub_dir:  bool,
    pub exclusive:   bool,
    /// Flag is intended to open an existing sub-database which was created with
    /// unknown flags In such cases, instead of returning the
    /// `MDBX_INCOMPATIBLE` error, the sub-database will be opened with
    /// flags which it was created, and then an application could determine the
    /// actual flags.
    pub accede:      bool,
    pub mode:        Mode,
    pub no_rdahead:  bool,
