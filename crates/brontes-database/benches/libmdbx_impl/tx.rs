...
use super::{cursor::LibmdbxCursorBench, utils::decode_one};
use crate::setup::tables::{BenchTables, BENCH_NUM_TABLES};

pub struct LibmdbxTxBench<K: TransactionKind> {
    /// Libmdbx-sys transaction.
    pub inner:             Transaction<K>,
    /// Database table handle cache.
    pub(crate) db_handles: Arc<RwLock<[Option<DBI>; BENCH_NUM_TABLES]>>,
}

impl LibmdbxTxBench<RO> {
    pub(crate) fn new_ro_tx(env: &DatabaseEnv) -> eyre::Result<LibmdbxTxBench<RO>, DatabaseError> {
        Ok(Self {
            inner:      env
                .begin_ro_txn()
...
impl LibmdbxTxBench<RW> {
    pub(crate) fn new_rw_tx(env: &DatabaseEnv) -> Result<LibmdbxTxBench<RW>, DatabaseError> {
        Ok(Self {
            inner:      env
                .begin_rw_txn()
                .map_err(|e| DatabaseError::InitTx(e.into()))?,
...
impl<K: TransactionKind> LibmdbxTxBench<K> {
    /// Gets a table database handle if it exists, otherwise creates it.
    pub fn get_dbi<T: Table>(&self) -> Result<DBI, DatabaseError> {
        let mut handles = self.db_handles.write();

        let table = BenchTables::from_str(T::NAME)
            .expect("Requested table should be part of `BenchTables`.");
    ...
    /// Create db Cursor
    pub fn new_cursor<T: Table>(&self) -> Result<LibmdbxCursorBench<T, K>, DatabaseError> {
        let inner = self
            .inner
            .cursor_with_dbi(self.get_dbi::<T>()?)
            .map_err(|e| DatabaseError::InitCursor(e.into()))?;

        Ok(LibmdbxCursorBench::new(inner))
    }
}

impl<K: TransactionKind> DbTx for LibmdbxTxBench<K> {
    type Cursor<T: Table> = LibmdbxCursorBench<T, K>;
    type DupCursor<T: DupSort> = LibmdbxCursorBench<T, K>;
...
impl DbTxMut for LibmdbxTxBench<RW> {
    type CursorMut<T: Table> = LibmdbxCursorBench<T, RW>;
    type DupCursorMut<T: DupSort> = LibmdbxCursorBench<T, RW>;
