...
fn address_abi_mapping(mapping: Vec<(ProtocolDetails, bool, bool)>) {
    ...
    for (map, has_functions, _) in mapping {
        ...
        if map.classifier_name.is_none() {
            ...
            writeln!(
                &mut file,
                "
                pub static {}: Lazy<(Option<&'static str>,StaticBindings)> = Lazy::new(|| (None, \
            ...
            writeln!(
                &mut file,
                "pub static {}: Lazy<(Option<&'static str>, StaticBindings)> = Lazy::new(|| \
                 (Some({}), StaticBindings::{}({}_Enum::None)));",
                name.to_uppercase(),
                name,
                name,
                name
    ...
    writeln!(
        &mut file,
        "pub static PROTOCOL_ADDRESS_MAPPING: phf::Map<[u8; 20], &'static Lazy<(Option<&'static \
         str>,StaticBindings)>> = \n{};\n",
        phf_map.build()
    )
    .unwrap();
