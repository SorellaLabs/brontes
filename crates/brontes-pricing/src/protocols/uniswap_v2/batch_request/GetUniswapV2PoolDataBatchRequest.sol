...
contract GetUniswapV2PoolDataBatchRequest {
    ...
    function data_constructor(address[] memory pools) public view returns (PoolData[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            (poolData[i].reserve0, poolData[i].reserve1, ) = IUniswapV2Pair(pool).getReserves();
            poolData[i].tokenA = IUniswapV2Pair(pool).token0();
            poolData[i].tokenADecimals = ERC20(IUniswapV2Pair(pool).token0()).decimals();
            poolData[i].tokenB = IUniswapV2Pair(pool).token1();
            poolData[i].tokenBDecimals = ERC20(IUniswapV2Pair(pool).token1()).decimals();
        }
        return poolData;
    }
}
