...
//     libmdbx::{tables::PoolCreationBlocks, utils::protocol_info,
// AddressToProtocolInfo, Libmdbx},     Pair,
// };
// use brontes_pricing::AllPairGraph as PairGraph;
// use brontes_types::{constants::USDT_ADDRESS, exchanges::Protocol,
// serde_utils::static_bindings}; use criterion::{
...
// BRONTES_DB_PATH in .env");     Libmdbx::init_db(brontes_db_endpoint,
// None).unwrap() }
//
...
//         let Some((block, res)) = next_val.map(|val| (val.0, val.1)) else {
// break 'outer };
//
...
//     let (_, fifty_thousand) = load_amount_of_pools_starting_from(&db, 0,
// 50_000);
//
//     g.bench_function("50_000 pool graph", move |b| {
//         b.iter(||
// black_box(PairGraph::init_from_hashmap(fifty_thousand.clone())))     });
//
//     let (_, hundred_thousand) = load_amount_of_pools_starting_from(&db, 0,
// 100_000);
//
//     g.bench_function("100_000 pool graph", move |b| {
//         b.iter(||
// black_box(PairGraph::init_from_hashmap(hundred_thousand.clone())))     });
//
//     let (_, two_hundred_thousand) = load_amount_of_pools_starting_from(&db,
// 0, 200_000);
//
//     g.bench_function("200_000 pool graph", move |b| {
//         b.iter(||
// black_box(PairGraph::init_from_hashmap(two_hundred_thousand.clone())))
//     });
//
//     let (_, all_known_pools) = load_amount_of_pools_starting_from(&db, 0,
// usize::MAX);     g.bench_function("all known pool graph", move |b| {
//         b.iter(||
// black_box(PairGraph::init_from_hashmap(all_known_pools.clone())))     });
// }
//
...
//     let db = init_bench_harness();
//
//     let (end_block, hundred_thousand) =
// load_amount_of_pools_starting_from(&db, 0, 100_000);     let graph =
// PairGraph::init_from_hashmap(hundred_thousand);     let (_, new_entries) =
// load_amount_of_pools_starting_from(&db, end_block + 1, 5000);
//     bench_insertions("100_000 pool graph inserting 5000 new pools", graph,
// &mut g, new_entries);
//
//     let (end_block, two_hundred_thousand) =
// load_amount_of_pools_starting_from(&db, 0, 200_000);     let graph =
// PairGraph::init_from_hashmap(two_hundred_thousand);     let (_, new_entries)
// = load_amount_of_pools_starting_from(&db, end_block + 1, 5000);
//     bench_insertions("200_000 pool graph inserting 5000 new pools", graph,
// &mut g, new_entries); }
//
// fn bench_insertions(
//     name: &str,
...
//
//     let (_, fifty_thousand) = load_amount_of_pools_starting_from(&db, 0,
// 100_000);     println!("loaded from db");
...
//
//     let (_, two_hundred_thousand) = load_amount_of_pools_starting_from(&db,
// 0, 200_000);     bench_yen_path_search(
...
//
//     let (_, all_known_pools) = load_amount_of_pools_starting_from(&db, 0,
// usize::MAX);     bench_yen_path_search(
...
// }
//
// fn bench_yen_path_search(name: &str, mut graph: PairGraph, g: &mut
// BenchmarkGroup<'_, WallTime>) {     let copy_graph = graph.clone();
//
//     g.bench_function(name, move |b| {
//         b.iter_batched(
//             || {
...
//                     black_box(graph.get_paths(pair, 18800000, 100, 5));
//                 }
//             },
...
//
// criterion_main!(graph_building_benches, graph_insertions_benches,
// yen_graph_path_search_benches);
//
// fn group<'a>(c: &'a mut Criterion, group_name: &str) -> BenchmarkGroup<'a,
// WallTime> {     let mut g = c.benchmark_group(group_name);
