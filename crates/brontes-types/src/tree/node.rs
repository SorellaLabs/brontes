...
impl Node {
    ...
    {
        let TreeSearchArgs { collect_current_node, child_node_to_collect } = find(self, &*nodes);
        ...
        let lower_classification_results = self
            .inner
            .iter_mut()
            .map(|node| node.modify_node_if_contains_childs(find, modify, nodes))
            .collect::<Vec<_>>();
        ...
        F: Fn(Vec<&mut Self>, &mut NodeData<V>),
    {
        ...
        let lower_has_better_collect = self
            .inner
            .iter_mut()
            .map(|n| n.modify_node_spans(find, modify, nodes))
            .collect::<Vec<_>>();
        ...
        if !all_lower_better {
            ...
            let mut node = vec![unsafe { &mut *(self as *mut Self) }];
