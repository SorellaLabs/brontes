...
pub mod test {
    ...
    use brontes_types::{
        normalized_actions::Action, tree::{BlockTree,remove_swap_transfers}, TreeSearchBuilder,
    };
    ...
    pub async fn test_transfer_de_duplication() {
        ...
        let transfers: Vec<Action> = tree.collect(&tx_hash, search_args).collect::<Vec<_>>();
