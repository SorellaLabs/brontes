...
impl CexDexInspector {
    ...
    pub fn cex_dex_profit_no_gas(
        ...
        for actions in swaps.iter() {
            for action in actions
                .iter()
                .map(|action| self.get_cex_dex(swaps, metadata))
            {
                if let Actions::Swap(swap) = action {
                    // think it should be fine to unwrap here because it should never fail but we
                    // never know
                    ...
                    if *cex_price1 > dex_price && *cex_price2 > dex_price {
                        //TODO: very terroristic, will finish tmrw should be
                        // chill
                    }
                }
            }
    ...
    pub fn get_cex_dex(
        ...
        let arb = &self.rational_dex_price(&swap, metadata).and_then(
            |(dex_price, cex_price1, cex_price2)| {
...
//TODO(WILL) I think we should reorganise the way we do priority fee, becuase
// why force a unecessary call on all inspectos when we can just TODO calc &
// store it when we build the tree, because all inspectors need it (same as
// bribe amount)
