...
use malachite::{
    num::{
        arithmetic::traits::ReciprocalAssign,
...
impl CexPriceMap {
    ...
    /// Computes a volume weighted average markout based on markouts in the
    /// assumed hedging period markouts - tuple of markout and volume,
    /// includes all positive in-slot markouts and all post-slot markouts
    pub fn get_vwam(&self, markouts: &Vec<&(Rational, Rational)>) -> Rational {
        let mut i: u64 = 0;
        let bindings = markouts.clone();
        let (last_markout, _) = bindings.get(0).clone().unwrap();
        let mut last_markout = last_markout.clone();
        let mut hedge_period = 0;
        let mut total_reversion: Rational = Rational::ZERO;
        let mut last_largest_avg = Rational::ZERO;
        for (markout, _) in markouts.iter() {
            let mut this_markout = markout.clone();
            if *markout < 0 {
                this_markout = Rational::ZERO
            }
            let markout_reversion = last_markout - this_markout.clone();
            last_markout = this_markout.clone();
            i += 1;
            if markout_reversion > 0 {
                total_reversion += markout_reversion;
                let this_avg = total_reversion.clone() / Rational::const_from_unsigned(i);
                if this_avg > last_largest_avg {
                    last_largest_avg = this_avg;
                    hedge_period = i;
                }
            };
        }
        let mut total_volume = Rational::ZERO;
        let mut total_volume_weighted_markout = Rational::ZERO;
        let bindings = markouts.clone();
        let (hedge_markouts, _) = bindings.split_at(hedge_period as usize);
        for (markout, volume) in hedge_markouts.iter() {
            if *markout > 0 {
                total_volume += volume;
                total_volume_weighted_markout += volume * markout;
            }
        }
        return total_volume_weighted_markout / total_volume;
    }
...
pub struct ExchangeData {
    pub exchange: CexExchange,
    pub quotes:   (HashMap<u64, CexQuote>, HashMap<u64, CexQuote>),
    pub trades:   (Vec<Trade>, Vec<Trade>), /* For intermediary pairs (token0, token2) and
                                             * (token2, token1) - with 0 being the purchased
                                             * token and 1 being the swapped token - else second
                                             * element is empty */
}
