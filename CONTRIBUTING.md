...
Thanks for your interest in improving Brontes!
...
If you contribute to this project, your contributions will be made to the project under the GPL 2.0 license.

### Code of Conduct

The Brontes project adheres to the [Rust Code of Conduct][rust-coc]. This code of conduct describes the _minimum_ behavior
...
1. **By opening an issue:** For example, if you believe that you have uncovered a bug
   in brontes, creating a new issue in the issue tracker is the way to report it.
...
3. **By resolving issues:** Typically this is done in the form of either
   ...
   reviewable manner.
4. **By identifying missed mev**: For example if you believe that you have uncovered a bundle that brontes did not classify, creating a new issue in the issue tracker (using the custom template) is the way to report it.
   This allows us to improve our classifiers & inspectors to widen the scope of what we can classify.
...
\*asking questions in the [dev-telegram][dev-tg]\*\*.

As Brontes is still in heavy development, the documentation can be a bit scattered. The [crate docs][brontes-docs] are our
...
- The Brontes version you are on (and that it is up to date)
...
Pull requests are the way concrete changes are made to the code, documentation, and dependencies of Brontes.
...
If the change being proposed alters code, it is either adding new functionality to Brontes, or fixing existing, broken
functionality.
In both of these cases, the pull request should include one or more tests to ensure that Brontes does not regress in the
future.
...
e.g. `cargo test mytest`) navigate to the directory of that file or use
the `-p <package-name>` option to run specific tests of a crate from anywhere in the
workspace (`cargo test -p brontes-classifier mytest`).
...
**Any Brontes community member is welcome to review any pull request**.
...
1. Does this change make sense for Brontes?
2. Does this change make Brontes better, even if only incrementally?
...
can typically be fixed by the Brontes maintainers merging the pull request, but they can also be an opportunity for the
...
of the pull request. Yes, we may merge a particular change that makes Brontes better, but the individual might just not
want to have anything to do with Brontes ever again. The goal is not just having good code.
...
[dev-tg]: https://t.me/sorella_brontes
[brontes-docs]: https://sorellalabs.github.io/brontes
